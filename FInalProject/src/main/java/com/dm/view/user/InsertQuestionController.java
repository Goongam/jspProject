...
public class InsertQuestionController extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if (qustion_id == -1)
			response.sendRedirect(request.getContextPath() + "/error.jsp"); // insert 실패
		else
			response.sendRedirect("Question.do?qustionid=" + qustion_id); // 질문id값으로 질문보기화면으로 이동
